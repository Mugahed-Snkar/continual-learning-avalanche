...
def main():
    ...
    benchmark = SplitCIFAR10(
        ...
        return_task_id=False,     # Single-head scenario
        seed=1234
    )
    ...
    strategy = EWC(
        ...
        optimizer=optimizer,
        criterion=criterion,
        ewc_lambda=0.4,          # Regularization strength (tuneable)
        train_mb_size=32,
        train_epochs=1,
        eval_mb_size=32,
        device=device,
    ...
    for exp_id, acc, forg in results:
        ...
        print(f"{exp_id:^10} | {acc_val:^17.4f} | {forg_val:^20.4f}")

    # Compute summary metrics
    AA = sum(all_acc) / len(all_acc)
    AIA = sum([acc for acc in all_acc]) / len(results)
    FM = sum(all_forg) / len(all_forg)
